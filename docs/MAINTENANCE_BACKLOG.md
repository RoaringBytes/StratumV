...
| 12 | How-to overlay: F1 toggle + first user topics | M | done | 2026-07-03: sv::HelpOverlay in src/engine/ui (engine defaults: Getting started, Camera); skinned_test appends Editing, Panels, Quitting via addTopic. F1 toggles; --show-help opens at startup (also the visual verification hook via the capture rig). Drawn only in the interactive frame path, so goldens untouched. Next: item 13 developer topics tab. |
